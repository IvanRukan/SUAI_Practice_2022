...
def words_from_digits(filename):  # ФУНКЦИЯ ДЛЯ РАЗДЕЛЕНИЯ СЛОВ И ЦИФР
    ...
    return len_for_words_in_func, digits_in_func, text_to_extract, words  # возвращаем из функции массив с длинами слов, массив цифр и исходный текст
...
def writer(len_for_words, digits, text, time, to_write):  # ФУНКЦИЯ ДЛЯ ЗАПИСИ РЕЗУЛЬТАТА В ФАЙЛЫ
    separated_temp = ''  # временная переменная для элементов сортировки
    result = open('A:\\учеба\\для практики\\result.txt', 'w', encoding='utf-8')  # открываем/создаем файл и переписываем его содержимое
    set_of_words = sorted(set(len_for_words), reverse=True)  # удаляем повторяющиеся элементы
    print(set_of_words)
    result.write('Слова по длине:\n')  # для наглядности пишем пояснение в файл
    for each in set_of_words:  # записываем слова по их длине
        for word in to_write:
            if len(word) == each:
                result.write(word + ' ')
        result.write('\n')
    set_of_digits = sorted(set(digits), reverse=True)   # записываем массив цифр
    print(set_of_digits)
...
len_for_words, digits, text, words_themselves = words_from_digits(f'{name}.txt')   # открытие исходного файла и первый этап сортировки
...
writer(len_for_words, digits, text, end_time, words_themselves)  # запись результата в файлы
